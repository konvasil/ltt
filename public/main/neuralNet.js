...
function drawTrainedData () {
    ...
    for(let i=0; i<data.length; i++){
        ...
        let y1 = map(data[i].xs.y, 0, 1, 0, height/2)
        let radius = 12;
        e = ellipse(x1+radius, y1 + radius, radius)
        text(JSON.stringify(brain.data.training[i].ys.freq.toFixed(2)), x1+2 + radius, y1+2 + radius) /*appears as normalized data after training (0.0 - 1.0)*/
        ...
        for(let j=0; j<data.length; j++){
            ...
            let noiseVal = noise((x1 + i) * noiseScale, y1 * noiseScale)
            stroke(noiseVal * 255, 18)
            //bezier(x1, y1, x2, y2, x3, y3, x1, y1, x2, y2, x3, y3)
            line(x1+2+radius, y1+2+radius, x2+2+radius, y2+2+radius)
        }
    }
     if (frameCount % 500 == 0){
            background(255)
